...
- [ ] Port 4001 tersedia (tidak digunakan aplikasi lain)
- [ ] Dependencies sudah terinstall (`npm install`)
- [ ] Server sudah running (`npm start`)
- [ ] Browser bisa akses http://localhost:4001 (atau http://YOUR_VPS_IP:4001)
- [ ] Test data sudah berhasil (`node test-send-data.js`)
- [ ] Firewall VPS sudah dibuka untuk port 4001 (jika deploy ke VPS)
...
```javascript
fetch('http://YOUR_VPS_IP:4001/api/mo/receive', {
  method: 'POST',
  headers: {
...
1. **View List MO**
   - Buka http://localhost:4001 (atau http://YOUR_VPS_IP:4001)
...
```bash
# Cek apakah port 4001 sudah digunakan
# Windows:
